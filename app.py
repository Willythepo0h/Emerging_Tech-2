...
# Load the model and use `@st.cache` decorator
...
# Replace this with an appropriate image path or remove it
# image = Image.open('Weather_girl.jpg')
# st.image(image, caption='Weather Classification Model - John Willard S. Sucgang')

# You might want to remove the above image and let users upload their own images

# Use `st.columns` to create a layout for displaying the image and prediction
col1, col2 = st.columns(2)
image = col1.file_uploader("Choose a photo from your computer", type=["jpg", "png"])
...
if image is None:
    ...
    if image.type not in ['image/jpeg', 'image/png']:
